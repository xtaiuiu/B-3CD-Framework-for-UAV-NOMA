...
def run(n_repests=1):
    step, end, n_points = 25, 200, 7
    n_ues = 25
    start = end - step * (n_points - 1)
    widths = np.arange(start, end + 1, step)
    obj_avg = pd.DataFrame({'NOMA': np.zeros(len(widths)),
                            'OMA': np.zeros(len(widths)),
                            'Fixed_power': np.zeros(len(widths)),
                            'Fixed_band': np.zeros(len(widths)),
                            'SHIO': np.zeros(len(widths))})
    for repeat in range(n_repests):
        t_r = time.perf_counter()
        sc = create_scenario(n_ues, 100)
        print(f" ####################### repeat = {repeat} ##############################")
        df = pd.DataFrame({'NOMA': np.zeros(len(widths)),
                           'OMA': np.zeros(len(widths)),
                           'Fixed_power': np.zeros(len(widths)),
                           'Fixed_band': np.zeros(len(widths)),
                           'SHIO': np.zeros(len(widths)),})
        for i in range(len(widths)):
            print(f" ####################### repeat = {repeat}, bandwidth = {widths[i]} ##############################")
            np.set_printoptions(formatter={'float': '{:0.4f}'.format})
            logging.disable(logging.INFO)
            sc.pn.b_tot = widths[i]
            ...
            # 4. SHIO
            sc.reset_scenario()
            f_heu, _ = NOMA_heuristic(sc)
            df.iloc[i, 4] = f_heu
        print(f" ############## repeat = {repeat} in {time.perf_counter() - t_r} seconds ######################")
        obj_avg += df
    obj_avg /= n_repests
    obj_avg.to_excel('bandwidth_compare.xlsx')

    # plot
    fontsize = 16
    plt.rcParams.update({'font.size': fontsize})
    fig1, ax1 = plt.subplots(figsize=(8, 6))
    obj_avg.plot(ax=ax1, legend=True, lw=2, xlabel='Total bandwidth (MHz)', ylabel='Max-min user rate',
                 fontsize=fontsize, style=["r-s", "m-.d", "c-p", 'k-^', 'g:o'], markersize=10, grid=True,
                 markerfacecolor='none')
    plt.show()

def plot():
    obj_avg = pd.read_excel('bandwidth_compare.xlsx', index_col=0)
    ...
    obj_avg.plot(ax=ax1, legend=True, lw=2, xlabel='Total bandwidth (MHz)', ylabel='Max-min user rate (Mbps)',
                 fontsize=fontsize, style=["r-s", "m-.d", "c-p", 'k-^', 'g:o'], markersize=10, grid=True,
                 markerfacecolor='none')
    x_ticks = [50, 75, 100, 125, 150, 175, 200]
